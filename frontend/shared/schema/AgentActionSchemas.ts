...
// Generate image (Higgsfield TTI → pasted as tldraw image shape)
export const GenerateImageAction = z
	.object({
		...
		prompt: z.string(),
		model: z
			.enum(['flux-2-pro', 'reve', 'soul-standard', 'z-image', 'default'])
			.optional()
			.describe(
				'Higgsfield model preset (server maps to platform model_id path). Omit or use default for server default.'
			),
		x: z.number(),
		y: z.number(),
		targetWidth: z
			...
			.optional()
			.describe(
				'If set together with targetHeight: (x,y) is the TOP-LEFT of a placement rectangle; the image is scaled with aspect ratio preserved to fit inside that box (centered). Use the same x,y,w,h as a red frame shape so the image lines up inside it.'
			),
		...
		frame: z
			...
			.optional()
			.describe(
				'behind: draw a rectangle behind the final image (good for a visible border/card). none: image only.'
			),
		framePadding: z
			.number()
			.optional()
			.describe('Extra space around the image for the behind frame, in page pixels. Default 8.'),
		frameColor: z
			.string()
			.optional()
			.describe('tldraw color name for the frame, e.g. red, orange, light-red. Default red.'),
	})
	.meta({
		title: 'Generate image',
		description:
			'Generate a bitmap via Higgsfield and place it on the canvas. Without targetWidth/targetHeight, (x,y) is the center point (like a normal paste). With targetWidth+targetHeight, (x,y) is the top-left of a box and the image is contain-fitted inside. Use frame=behind to add a rectangle behind the image sized to the image (+ padding). To put an image inside an existing box: use that box\'s x,y and its width/height as targetWidth/targetHeight.',
		_systemPromptCategory: 'edit',
	})
