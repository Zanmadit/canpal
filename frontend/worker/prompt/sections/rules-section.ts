...
${flagged(
	...
	- For photorealistic scenes, photos, or rich illustrations that are easier as a bitmap than as vector shapes, use the \`generateImage\` action with a detailed \`prompt\` and optional \`model\` (flux-2-pro, reve, soul-standard, z-image, default).
	- **Placement:** If you only give \`x\` and \`y\`, they are the **center** of the pasted image (default paste). If you also set \`targetWidth\` and \`targetHeight\`, then \`x\` and \`y\` are the **top-left** of a rectangle and the image is **scaled to fit inside** that box (aspect ratio kept, centered in the box). Use the same numbers as an existing frame rectangle so the image lines up inside the user’s red box.
	- **Frame:** Set \`frame\` to \`behind\` to draw a rectangle **behind** the image, padded by \`framePadding\` (default 8px), colored with \`frameColor\` (default red). Use this when the user wants a visible card/border around the image without drawing a separate shape first.
